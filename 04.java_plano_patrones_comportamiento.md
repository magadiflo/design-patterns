...
El patrón `Chain of Responsibility` (`Cadena de Responsabilidad`) es un patrón de comportamiento que permite pasar una
solicitud a lo largo de una cadena de manejadores. Cada manejador decide si procesa la solicitud o la pasa al siguiente
en la cadena.
...
### 💼 Casos comunes en lógica de negocio

- Validación de datos antes de persistir (campos requeridos, formato, reglas de dominio).
- Flujo de autorización: autenticación → autorización → verificación de contexto.
- Aprobaciones jerárquicas (ej: solicitud de crédito, vacaciones).
- Procesamiento de operaciones complejas por etapas (ETL, preprocesamiento, auditoría, etc.).
...
- `Flexibilidad`: puedes reordenar la cadena o cambiarla dinámicamente.
...
3. Que el monto total no supere cierto límite.
4. Que la dirección de envío sea válida.
