...
> También llamado: `Observador`, `publicación-suscripción`, `model-patrón`, `event-subscriber`, `listener`.
...
6. El `Client` crea objetos tipo `publisher` y `subscriber` por separado y después registra a los suscriptores para las
   actualizaciones del `publisher`.

### Componentes del Patrón y su Terminología

Aunque en el diagrama UML anterior ya hemos visto los componentes del patrón `Observer`, en esta lección lo veremos un
poco más a detalle.

Aquí tienes los componentes clave del patrón y los nombres que usaremos en nuestra implementación, junto con sus
equivalentes para tu referencia.
