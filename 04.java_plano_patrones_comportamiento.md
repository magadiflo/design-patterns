...
📌 Ejemplos clásicos: `Strategy`, `Template Method`, `Observer`, `Command`, `Chain of Responsibility`, `State`, etc.
...
> También llamado: `Observador`, `publicación-suscripción`, `model-patrón`, `event-orderSubscriber`, `listener`.

El patrón `Observer` es un patrón de diseño de tipo comportamiento que define una dependencia uno-a-muchos entre
objetos. Cuando el estado de un objeto (`Subject` o `Publisher`) cambia, todos sus
dependientes (`Observers` o `Subscribers`) son notificados y actualizados automáticamente.

La esencia del patrón radica en el `desacoplamiento`: los objetos que cambian de estado y los objetos que reaccionan
a ese cambio están separados. El `Publisher` no necesita saber la identidad o la implementación de sus `Subscribers`,
solo necesita saber que existen y que puede notificarlos a través de una interfaz común.

### 📐 Representación UML del patrón Observer

![07.png](assets/02-java-plano-comportamiento/07.png)

1. El `Publisher` envía eventos de interés a otros objetos. Estos eventos ocurren cuando el `publisher` cambia su
   estado o ejecuta algunos comportamientos. Los `publishers` contienen una infraestructura de suscripción que permite
   a nuevos y antiguos suscriptores abandonar la lista.
2. Cuando sucede un nuevo evento, el `publisher` recorre la lista de suscripción e invoca el método de notificación
   declarado en la interfaz suscriptora en cada objeto suscriptor.
3. La interfaz `Subscriber` declara la interfaz de notificación. En la mayoría de los casos, consiste en un único
   método `update(...)`. El método puede tener varios parámetros que permitan al `publisher` pasar algunos detalles del
   evento junto a la actualización.
4. Los `Concrete Subscribers` realizan algunas acciones en respuesta a las notificaciones emitidas por el `publisher`.
   Todas estas clases deben implementar la misma interfaz de forma que el `publisher` no esté acoplado a clases
   concretas.
5. Normalmente, los suscriptores necesitan cierta información contextual para manejar correctamente la actualización.
   Por este motivo, a menudo los `publishers` pasan cierta información de contexto como argumentos del método de
   notificación. El `publisher` puede pasarse a sí mismo como argumento, dejando que los suscriptores extraigan la
   información necesaria directamente.
6. El `Client` crea objetos tipo `publisher` y `orderSubscriber` por separado y después registra a los suscriptores para
   las
   actualizaciones del `publisher`.

### Componentes del Patrón y su Terminología

Aunque el diagrama UML del GoF nos presenta la estructura básica del patrón Observer, en nuestra implementación práctica
necesitamos comprender tanto los **componentes abstractos** como sus **implementaciones concretas**.

**En el diagrama UML teórico vemos:**

- `Publisher` (Subject abstracto)
- `Subscriber` (Observer interfaz)

**En la implementación práctica agregamos:**

- `ConcretePublisher` (implementación específica del dominio)
- `ConcreteSubscriber` (observers con lógica de negocio específica)

### Decisión de Diseño: ¿Clase Abstracta o Concreta?

Nuestro sistema de pedidos lo estábamos creando de la siguiente manera `Publisher <-- OrderPublisher`, es decir,
creando una clase abstracta llamada `Publisher` y su respectiva implementación concreta `OrderPublisher`, pero
luego de analizar, he optado por **eliminar la abstracción innecesaria** y crear directamente una clase
`OrderPublisher` concreta, ya que:

- ✅ Solo manejamos un dominio (pedidos)
- ✅ No necesitamos múltiples tipos de publishers
- ✅ Evitamos sobreingeniería (KISS principle)
- ✅ Código más directo y mantenible

> **Nota:** Si tuviéramos múltiples dominios (`OrderPublisher`, `ProductPublisher`, `UserPublisher`), entonces sí
> justificaría una clase abstracta `Publisher` para reutilizar código común.

1. `Publisher (Sujeto / Observable)`. El objeto que tiene el estado de interés. Mantiene una lista de sus suscriptores y
   proporciona métodos para que se puedan adjuntar, separar y notificar.
    - `Función`: Cuando ocurre un evento o su estado cambia, recorre su lista de suscriptores y les notifica el cambio,
      pasando los datos relevantes si es necesario.
    - También son conocidos como: `Subject`, `Observable`, `Dispatcher`.


2. `Subscriber (Observador / Listener)`. La interfaz que define el método de actualización (`update`) que debe ser
   llamado por el `Publisher` cuando su estado cambia.
    - `Función`: Es el contrato que deben seguir todos los objetos que deseen ser notificados.
    - También son conocidos como: `Observer`, `Listener`, `Handler`.


3. `Concrete Publisher (Sujeto Concreto)`. La implementación específica del `Publisher`. Almacena el estado que se
   observa y contiene la lógica para gestionar la lista de `Subscribers` y realizar la notificación.
    - `Función`: Es el objeto "real" que emite los eventos.
    - También son conocidos como: `ConcreteSubject`, `ConcreteObservable`.


4. `Concrete Subscriber (Observador Concreto)`. Las implementaciones específicas de la interfaz `Subscriber`. Cada una
   encapsula una lógica de negocio única para responder a la notificación del `Publisher`.
    - `Función`: Son los objetos que ejecutan la lógica de negocio al ser notificados.
    - También son conocidos como: `ConcreteObserver`, `ConcreteListener`, `ConcreteHandler`.

### 🎯 Ejemplo 01: Sistema de Procesamiento de Pedidos E-commerce

En un sistema de e-commerce, cuando un `pedido cambia de estado`, múltiples componentes del sistema necesitan
reaccionar automáticamente para ejecutar diferentes procesos de negocio. El desafío es mantener estos componentes
desacoplados pero coordinados.

Casos de Uso Específicos

1. `Pedido Confirmado (Estado: CONFIRMED)`. Cuando un cliente confirma y paga su pedido, el sistema debe ejecutar
   automáticamente:
    - 📧 Enviar email de confirmación al cliente
    - 📦 Actualizar inventario (reducir stock de productos)
    - 💰 Registrar transacción en el sistema contable
    - 📊 Actualizar métricas de ventas del dashboard
    - 📄 Generar factura en PDF y almacenarla


2. `Pedido Enviado (Estado: SHIPPED)`. Cuando el pedido sale del almacén:
    - 📧 Notificar al cliente con código de tracking
    - 📱 Enviar SMS de confirmación de envío
    - 🚚 Actualizar sistema de logística con información de transporte


3. `Pedido Cancelado (Estado: CANCELLED)`. Cuando se cancela un pedido:
    - 💳 Procesar reembolso automático
    - 📦 Restaurar inventario (devolver stock)
    - 📧 Notificar cancelación al cliente

`Objetivo del Patrón Observer`. Permitir que múltiples objetos (observers/listeners) se suscriban automáticamente a los
cambios de estado de un pedido (subject) sin que estos componentes se conozcan directamente entre sí, manteniendo el
sistema desacoplado, extensible y mantenible.

Entonces, la implementación de nuestro patrón `Observer` estará basado en el siguiente diagrama de clases.

![08.png](assets/02-java-plano-comportamiento/08.png)

### 📌 Definición de Clases de Dominio (Soporte para Observer)

Antes de implementar el patrón Observer, necesitamos definir los objetos de dominio que representarán el evento a
transmitir y el estado del pedido.
...
- Rol: Enumeración que modela los estados relevantes de un pedido en el sistema de e-commerce.
- Uso dentro del patrón:
    - Se utiliza en el evento (`OrderEvent`) para indicar qué tipo de cambio ocurrió.
    - Permite que cada Subscriber reaccione de manera distinta según el estado del pedido.
...
- Rol: Objeto que encapsula la información de un evento de pedido.
- Campos principales:
    - `orderId`: Identificador único del pedido.
    - `status`: Estado del pedido (`CONFIRMED`, `SHIPPED`, `CANCELLED`).
- Uso dentro del patrón:
    - Actúa como el mensaje que el Publisher envía a todos los Subscribers.
    - Permite que los Subscribers conozcan el contexto del cambio (qué pedido y en qué estado está).

✅ Con esto queda definido el modelo de evento que servirá como base para que el `Publisher` emita notificaciones y los
`Subscribers` reaccionen.

### 📌 Interfaz Subscriber y Suscriptores Concretos

En el `patrón Observer` (`Publisher–Subscriber`), los `Subscribers` son los objetos que se registran en el `Publisher`
para recibir notificaciones cuando ocurre un evento.
...
- Rol dentro del patrón:
    - Es la interfaz base que deben implementar todos los suscriptores.
    - Define el contrato: cada suscriptor debe poder reaccionar a un evento (`OrderEvent`).
- Método clave:
    - `update(OrderEvent orderEvent)`: llamado por el `Publisher` cada vez que ocurre un cambio en el pedido.

`Implementaciones concretas`

Cada clase implementa la interfaz `OrderSubscriber` y representa un proceso de negocio específico que debe ejecutarse
tras un cambio en el estado del pedido. Para efectos de simulación, se usa logging (log.info) en lugar de integraciones
reales (correos, DB, PDFs, etc.).
...
- Envía correos de confirmación, envío o cancelación.
- Simula un servicio de notificaciones por correo.
...
- Maneja la contabilidad del pedido.
- Registra transacciones cuando se confirma y procesa reembolsos cuando se cancela.
...
- Se encarga del stock de productos.
- Reduce inventario en pedidos confirmados y lo restaura en pedidos cancelados.
...
- Genera una factura en PDF al confirmar el pedido.
- Ejemplo clásico de acción automática tras una compra.
...
- Actualiza las métricas del dashboard de ventas.
- Aumenta ventas al confirmar y registra cancelaciones cuando ocurren.
...
- Envía notificaciones vía SMS cuando el pedido es despachado.
- Simula integración con un proveedor de mensajería.

✅ Con estas implementaciones vemos claramente la desacoplación: el `Publisher` no sabe nada de cómo se envía el correo,
cómo se restaura el inventario o cómo se genera la factura. Solo emite un evento, y cada `Subscriber` decide cómo
reaccionar.

### 📌 Publisher — OrderPublisher

El `Publisher` (también conocido como `Subject`) es el componente central del patrón `Observer`. Su rol es mantener una
lista de suscriptores y notificarlos automáticamente cuando se produce un cambio de estado en un pedido.
...
1. `Gestión de suscriptores`
    - `subscribe(OrderSubscriber)` → Permite registrar nuevos `Subscribers` interesados en los eventos de pedidos.
    - `unsubscribe(OrderSubscriber)` → Permite remover `Subscribers` cuando ya no necesitan ser notificados.

2. `Cambio de estado de pedidos`
    - `changeOrderStatus(String orderId, OrderStatus newStatus)` → Método principal que simula el cambio de estado de
      un pedido.
    - Crea un objeto `OrderEvent` con el `ID del pedido` y el `nuevo estado`.
    - Luego invoca a `notifySubscribers(...)`.

3. `Notificación a suscriptores`
    - `notifySubscribers(OrderEvent)` → Recorre todos los `Subscribers` y les envía el evento.
    - Cada `Subscriber` decide qué hacer con la información recibida.

✅ Con este `OrderPublisher`, tu implementación de `Observer` queda totalmente clara:

- El `Publisher` no sabe qué hacen los suscriptores, solo que deben ser notificados.
- Los `Subscribers` se pueden agregar o quitar en tiempo de ejecución sin modificar la lógica del `Publisher`.

### Cliente

En el patrón `Observer`, el `Cliente` es el responsable de configurar y orquestar el sistema. No implementa la lógica
de negocio, sino que se encarga de:

1. Instanciar el `Publisher` (`OrderPublisher`).
2. Crear los `Subscribers` que reaccionarán a los eventos (`EmailNotificationOrderSubscriber`,
   `AccountingOrderSubscriber`, etc.).
3. Registrar los `Subscribers` en el `Publisher` mediante `subscribe()`.
4. Simular cambios de estado en pedidos para observar cómo todos los suscriptores reaccionan automáticamente.
...
✅ Con esta clase, el patrón `Observer` queda demostrado de principio a fin:

- El `Client` solo dispara eventos (`CONFIRMED`, `SHIPPED`, `CANCELLED`).
- El `Publisher` se encarga de notificar.
- Cada `Subscriber` reacciona de manera autónoma, sin que el cliente sepa nada de sus implementaciones.

### Ejecutando patrón Observer

Cuando ejecutamos la clase `Client`, podemos observar cómo el `Publisher (OrderPublisher)` coordina el cambio de
estado de los pedidos y notifica automáticamente a todos los `Subscribers` registrados.

Cada escenario (`CONFIRMED`, `SHIPPED`, `CANCELLED`) dispara un `evento (OrderEvent)`, el cual es propagado a todos los
observadores interesados.

````bash
publisher.OrderPublisher -- Nuevo subscriber agregado: AccountingOrderSubscriber
publisher.OrderPublisher -- Nuevo subscriber agregado: EmailNotificationOrderSubscriber
publisher.OrderPublisher -- Nuevo subscriber agregado: InventoryOrderSubscriber
publisher.OrderPublisher -- Nuevo subscriber agregado: InvoiceGeneratorOrderSubscriber
publisher.OrderPublisher -- Nuevo subscriber agregado: SalesMetricsOrderSubscriber
publisher.OrderPublisher -- Nuevo subscriber agregado: SMSNotificationOrderSubscriber
Client -- === Pedido CONFIRMADO ===
publisher.OrderPublisher -- Cambio de estado [Pedido: ORD-001 -> estado: CONFIRMED]
publisher.OrderPublisher -- Notificando a todos los Subscribers...
subscriber.AccountingOrderSubscriber -- Contabilidad: Transacción registrad para ORD-001
subscriber.EmailNotificationOrderSubscriber -- Email enviado: Confirmación de pedido ORD-001
subscriber.InventoryOrderSubscriber -- Inventario: Stock reducido para pedido ORD-001
subscriber.InvoiceGeneratorOrderSubscriber -- Factura: PDF generado para pedido ORD-001 -> INV-ORD-001.pdf
subscriber.SalesMetricsOrderSubscriber -- Métricas: Venta confirmada registrada - ORD-001
publisher.OrderPublisher -- Notificación completada
Client -- === Pedido ENVIADO ===
publisher.OrderPublisher -- Cambio de estado [Pedido: ORD-002 -> estado: SHIPPED]
publisher.OrderPublisher -- Notificando a todos los Subscribers...
subscriber.EmailNotificationOrderSubscriber -- Email enviado: Su pedido ORD-002 está en camino
subscriber.SMSNotificationOrderSubscriber -- SMS enviado: Su pedido ORD-002 fue despachado - Track: TRKORD-002
publisher.OrderPublisher -- Notificación completada
Client -- === Pedido CANCELADO ===
publisher.OrderPublisher -- Cambio de estado [Pedido: ORD-003 -> estado: CANCELLED]
publisher.OrderPublisher -- Notificando a todos los Subscribers...
subscriber.AccountingOrderSubscriber -- Contabilidad: Reembolso procesado para ORD-003
subscriber.EmailNotificationOrderSubscriber -- Email enviado: Cancelación de pedido ORD-003
subscriber.InventoryOrderSubscriber -- Inventario: Stock restaurado para pedido ORD-003
subscriber.SalesMetricsOrderSubscriber -- Métricas: Venta cancelada registrada - ORD-003
publisher.OrderPublisher -- Notificación completada 
````

➡️ Esto permite que el sistema sea extensible: si mañana se requiere un nuevo proceso (ej. integración con WhatsApp
Business), simplemente se implementa un nuevo `OrderSubscriber` y se suscribe al `Publisher`, sin necesidad de
