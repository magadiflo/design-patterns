...
El `ValidationResult` representa el resultado de la validación individual de un registro.
...
La clase `FileProcessorException` es una excepción personalizada utilizada para representar errores internos en el
...
Devuelve un objeto `FileStorageInfo` con los metadatos del archivo (ubicación, nombre, tipo de almacenamiento).
...
Mediante Spring, todos los beans que extienden `FileProcessor` son inyectados automáticamente como una lista. La fábrica
...
### 🌐 FileProcessorController – Exposición del procesamiento vía REST
...
### 🧪 Caso de prueba: Procesamiento exitoso de archivo .txt
...
- `Inyección de la cadena de validación`
    - Se inyecta el bean `orderValidationChain` configurado previamente con la cadena de validadores.
    - Esto garantiza que la validación se ejecute en orden y de forma transparente.


- `Método processOrder`
    - Recibe la orden de compra.
    - Registra datos relevantes para auditoría mediante logs.
    - Ejecuta la cadena de validación con `handle(order)`. Si alguna validación falla, se lanza una excepción que puede
      ser manejada globalmente.
    - Si pasa todas las validaciones, simula el procesamiento exitoso y retorna un `OrderResponse` con información de
      confirmación.


- `Extensibilidad`
    - La lógica de persistencia, notificación o reserva de stock está indicada para futuras implementaciones, separando
      responsabilidades.


- `Generación simple de ID de orden`. Se crea un ID único basado en la marca de tiempo actual.

### Controlador REST para procesamiento de órdenes

El controlador expone un endpoint REST que recibe solicitudes para crear órdenes de compra.

Valida automáticamente los datos recibidos y delega el procesamiento completo al servicio encargado, que ejecuta la
cadena de validaciones antes de confirmar la orden.

Finalmente, responde con un estado HTTP 201 y el resultado del procesamiento.
...
- Endpoint `POST /api/v1/orders`:
    - Recibe una solicitud con el DTO `PurchaseOrder` en el cuerpo de la petición.
    - Usa `@Valid` para activar la validación automática basada en las anotaciones de los DTOs.
    - Registra con logs la recepción de la nueva orden para monitoreo.
    - Invoca el servicio `orderProcessingService.processOrder(order)` para procesar la orden, que incluye la cadena de
      validación.
    - Devuelve un `HTTP 201 Created` con el cuerpo que contiene el `OrderResponse` con el resultado.

### Probando validación de órdenes

En este apartado se muestran ejemplos prácticos de cómo se comporta el sistema ante solicitudes de orden que cumplen o
no con las reglas de negocio establecidas por la cadena de validación.

#### 🟢 Caso exitoso (todas las validaciones pasan)

Se envía una orden válida con cliente activo, stock disponible y dirección correcta.

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"customerId\": 1, \"items\": [{\"productId\": 101, \"quantity\": 1, \"unitPrice\": 1200}, {\"productId\": 103, \"quantity\": 1, \"unitPrice\": 89.90}], \"shippingAddress\": {\"street\": \"Av. Universitaria 123\", \"city\": \"Lima Gris\", \"postalCode\": \"15001\", \"country\": \"PERU\"}}" http://localhost:8080/api/v1/orders | jq
>
< HTTP/1.1 201
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 11 Aug 2025 21:42:55 GMT
<
{
  "orderId": "ORD-1754948575431",
...
````bash
d.m.p.s.b.c.o.c.OrderController          : Nueva orden recibida para el cliente: 1
m.p.s.b.c.o.s.OrderProcessingServiceImpl : ==== Procesando orden ====
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Customer ID: 1
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Items: 2
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Total amount: $ 1289.90
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Destination: Lima Gris
m.p.s.b.c.o.s.OrderProcessingServiceImpl : ===========================
.m.p.s.b.c.o.h.CustomerValidationHandler : [CustomerValidation] Verificando cliente con ID: 1
.m.p.s.b.c.o.h.CustomerValidationHandler : [CustomerValidation] Cliente válido: John Doe
d.m.p.s.b.c.o.h.StockValidationHandler   : [StockValidation] Verificando disponibilidad de productos
d.m.p.s.b.c.o.h.StockValidationHandler   : [StockValidation] Stock OK para Dell Laptop (Disponible: 5, Solicitado: 1)
d.m.p.s.b.c.o.h.StockValidationHandler   : [StockValidation] Stock OK para Mechanical Keyboard (Disponible: 10, Solicitado: 1)
d.m.p.s.b.c.o.h.AddressValidationHandler : [AddressValidation] Verificando: Lima Gris, PERU
d.m.p.s.b.c.o.h.AddressValidationHandler : [AddressValidation] Dirección válida para el envío
s.b.c.o.h.BusinessHoursValidationHandler : [BusinessHoursValidation] Verificando horario de procesamiento...
s.b.c.o.h.BusinessHoursValidationHandler : [BusinessHoursValidation] Horario válido para el procesamiento
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Orden procesada correctamente
m.p.s.b.c.o.s.OrderProcessingServiceImpl : El pedido ha sido registrado y está listo para su preparación
````

- En el caso exitoso, la cadena completa de validación se ejecuta sin errores y la orden es procesada
  satisfactoriamente.

#### 🔴 Caso de falla (stock insuficiente)

Se envía una orden con un producto que no tiene stock suficiente.

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"customerId\": 1, \"items\": [{\"productId\": 102, \"quantity\": 5, \"unitPrice\": 25.50}], \"shippingAddress\": {\"street\": \"Av. Universitaria 123\", \"city\": \"Lima Gris\", \"postalCode\": \"15001\", \"country\": \"PERU\"}}" http://localhost:8080/api/v1/orders | jq
>
< HTTP/1.1 400
< Content-Type: application/problem+json
< Transfer-Encoding: chunked
< Date: Mon, 11 Aug 2025 21:47:25 GMT
< Connection: close
<
{
  "type": "about:blank",
  "title": "Problemas en la validación de lógica de negocio",
  "status": 400,
  "detail": "Stock insuficiente para Wireless Mouse. Disponible: 0, Solicitado: 5",
...
m.p.s.b.c.o.s.OrderProcessingServiceImpl : ==== Procesando orden ====
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Customer ID: 1
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Items: 1
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Total amount: $ 127.50
m.p.s.b.c.o.s.OrderProcessingServiceImpl : Destination: Lima Gris
m.p.s.b.c.o.s.OrderProcessingServiceImpl : ===========================
.m.p.s.b.c.o.h.CustomerValidationHandler : [CustomerValidation] Verificando cliente con ID: 1
.m.p.s.b.c.o.h.CustomerValidationHandler : [CustomerValidation] Cliente válido: John Doe
d.m.p.s.b.c.o.h.StockValidationHandler   : [StockValidation] Verificando disponibilidad de productos
d.m.p.s.b.c.o.a.GlobalExceptionHandler   : Error de validación de negocio: Stock insuficiente para Wireless Mouse. Disponible: 0, Solicitado: 5

````

- En el caso de error, la validación de stock detecta la insuficiencia y se lanza una excepción que es capturada por el
  manejador global, retornando un error `HTTP 400` con detalles claros para el cliente.
- Los logs permiten seguir paso a paso el flujo de validación y procesamiento, facilitando la depuración.

---

# Observer

## 🎯 Ejemplo 01: Sistema de Procesamiento de Pedidos E-commerce

CHAT GTP: Aquí resume de qué trata el ejemplo que vaamos a ver, que es relacionado al patrón observer pero aplicado a un
ejemplo real del mundo laboral.

### Modelo y Evento de Dominio
...
- `Rol`: Define los posibles estados del pedido dentro del dominio.
- `Relación con el patrón Observer`: Actúa como dato clave en el evento que se propagará a los `subscribers`.
- `Nota`: Mantener los estados como un enum ayuda a garantizar consistencia en el sistema y facilita validar
  transiciones.

````java
/**
 * correlationId te permite trazar toda la cadena (útil en logs y APM).
 * occurredAt fija el tiempo del suceso
 */
...
- `Rol`: Representa el evento de dominio que viaja desde el `Publisher` hacia los `Subscribers`.
- Campos importantes:
    - `orderId`: Identificador único del pedido.
    - `status`: Estado actualizado (`CONFIRMED`, `SHIPPED`, `CANCELLED`).
    - `occurredAt`: Momento exacto del cambio de estado → útil para auditoría o métricas.
    - `correlationId`: Permite rastrear el evento a través de múltiples microservicios/logs (trazabilidad en APMs como
      NewRelic, Datadog, etc.).
- Método `of()`: Método de fábrica para simplificar la creación del evento, encapsulando la lógica de asignar el
  `Instant.now()`.

📌 Nota práctica sobre el `Patrón Observer en Spring`
> En el mundo real, este evento (`OrderStatusChangedEvent`) será el mensaje que se `"publica"` usando el
> `ApplicationEventPublisher` (`Publisher`) y que los distintos `Listeners` (`Subscribers`) recibirán.
>
> Esto mantiene el acoplamiento bajo: el servicio que genera el evento no sabe quién lo consumirá, solo lanza
> el evento.

### Configuración de Ejecución Asíncrona en Spring Boot
...
1. `@EnableAsync`
    - Habilita la ejecución de métodos asíncronos en la aplicación `Spring Boot`.
    - Esto significa que cualquier método anotado con `@Async` podrá ejecutarse en un hilo separado, sin bloquear el
      flujo principal de la aplicación.
    - En nuestro caso, lo aplicaremos a los `listeners` (`subscribers`) del `patrón Observer`, de manera que procesen
      los eventos sin afectar el flujo de negocio principal (ej. confirmar un pedido).


2. `Bean` `ordersExecutor`
    - Estás creando un bean de tipo `Executor` llamado `ordersExecutor` que `Spring` usará para ejecutar tareas
      marcadas con `@Async`. Gracias a `@EnableAsync`, Spring activa el soporte para ejecución asíncrona.
    - Puedes referenciarlo explícitamente en `@Async("ordersExecutor")`.
    - Define un `ThreadPoolTaskExecutor`, que es el pool de hilos encargado de procesar tareas en paralelo.
    - Parámetros importantes:
        - `setThreadNamePrefix("orders-")`: cada hilo creado llevará un prefijo reconocible (`orders-1`, `orders-2`,
          etc.), muy útil para depuración en logs.
        - `setCorePoolSize(4)`: cantidad mínima de hilos siempre activos.
        - `setMaxPoolSize(8)`: máximo de hilos que se pueden crear si hay alta carga.
        - `setQueueCapacity(200)`: número de tareas en espera que el pool puede almacenar si todos los hilos están
          ocupados.
        - `initialize()`, inicializamos el executor.


3. Ventajas prácticas
    - `Escalabilidad`: varios eventos pueden ser procesados en paralelo.
    - `Aislamiento`: si un listener falla o se demora, no bloquea al resto del flujo principal.
    - `Control fino`: al definir un `Executor` propio, tenemos control sobre el tamaño de pool y comportamiento bajo
      carga.

> 👉 En resumen: esta configuración asegura que los `subscribers` del `patrón Observer` (`listeners` en `Spring`)
> trabajen en segundo plano, usando un `thread pool` optimizado y aislado del flujo principal de negocio.

### 🧵 ¿Qué es un Executor?

Un `Executor` es un componente que permite ejecutar tareas de forma asíncrona, delegando su ejecución a un
`pool de hilos` configurado previamente. En lugar de crear y manejar hilos manualmente, simplemente definimos cómo
queremos que se comporten (cantidad de hilos, cola de tareas, prefijo de nombres, etc.), y el `Executor` se encarga
del resto.

Esto facilita la concurrencia controlada, mejora la escalabilidad y desacopla la lógica de negocio del manejo de hilos.

> En resumen: configuramos el `Executor` una vez, y él se encarga de ejecutar las tareas en segundo plano de forma
> eficiente y segura.

#### 🔄 ¿Qué tareas ejecuta?

Las tareas que ejecuta este `Executor` son aquellas que están referenciadas explícitamente mediante la anotación
`@Async`, indicando el nombre del bean: `@Async("ordersExecutor")`.

Esto significa que cualquier método marcado con esa anotación será ejecutado en segundo plano por el `pool de hilos`
definido en el bean `ordersExecutor`. Spring se encarga de interceptar la llamada y delegarla al `Executor`
correspondiente, sin que el desarrollador tenga que preocuparse por la gestión de hilos.
...
public void procesarPedido(Pedido pedido) {
    // Esta tarea se ejecutará de forma asíncrona usando el Executor configurado
}
````

> ⚙️ La magia ocurre por debajo: `Spring` detecta la anotación `@Async`, busca el `Executor` con ese nombre, y delega
> la ejecución del método a uno de sus hilos disponibles.

#### ⚙️ ¿Qué pasa si no se especifica un Executor en @Async?

Cuando se usa la anotación `@Async` sin indicar el nombre de un `Executor`, como en:

````java

@Async
public void procesarPedido(Pedido pedido) {
    // Esta tarea se ejecutará de forma asíncrona usando el Executor por defecto
}
````

`Spring` busca un `Executor` por defecto para ejecutar la tarea. Si no se ha definido explícitamente un bean llamado
`taskExecutor`, `Spring` crea uno automáticamente con una configuración básica (generalmente un
`SimpleAsyncTaskExecutor`, que no reutiliza hilos y puede generar uno nuevo por cada tarea).

🧠 ¿Qué implica esto?

- ✅ La tarea `sí se ejecuta de forma asíncrona`, pero con un comportamiento menos eficiente y menos controlado.
- ⚠️ No hay reutilización de hilos, lo que puede afectar el rendimiento en sistemas con alta concurrencia.
- 🔍 No se puede ajustar el tamaño del pool, la cola de tareas ni el prefijo de nombres de hilos.
- 📉 Puede dificultar la trazabilidad y el monitoreo si no se personaliza.

> 💡 Recomendación: Siempre que se trabaje en sistemas productivos o con carga concurrente significativa, es mejor
> definir un `ThreadPoolTaskExecutor` personalizado y referenciarlo explícitamente con `@Async("nombreDelExecutor")`.

### Publisher en Spring Boot: OrderService
...
1. `@Service` + `ApplicationEventPublisher`
    - `OrderService` representa el `Publisher` en el `patrón Observer`.
    - Usamos `ApplicationEventPublisher` de `Spring`, que es el mecanismo estándar para emitir eventos dentro del
      contexto de la aplicación.
    - Esto evita tener que gestionar manualmente una lista de `subscribers`, como hicimos en la versión pura de Java.


2. Método `changeOrderStatus(...)`
    - En un escenario real, este método contendría lógica de negocio transaccional (ej. actualizar el estado del pedido
      en la base de datos).
    - Una vez completada esa lógica, se construye y publica el evento de dominio `OrderStatusChangedEvent`.


3. Ventajas de este enfoque en Spring
    - `Desacoplamiento`: el `Publisher` no necesita conocer a los `Subscribers`. Cualquier `listener` registrado en el
      contexto recibirá el evento.
    - `Escalabilidad`: los listeners pueden ejecutarse en segundo plano (`@Async`) y manejar la carga en paralelo.
    - `Integración natural con Spring`: aprovecha el contenedor y el ciclo de vida de beans de Spring.

👉 Conclusión:
> En nuestra implementación, `OrderService` es el `Publisher`. Su única responsabilidad es emitir un evento de dominio
> cuando el estado del pedido cambia. Gracias a Spring, no necesita gestionar manualmente la lista de suscriptores ni
> la notificación; todo eso lo maneja el contenedor de eventos de Spring.

#### 📌 Diferencia entre `@EventListener` y `@TransactionalEventListener`

En Spring, los listeners de eventos pueden reaccionar de dos formas distintas, dependiendo del contexto en el que se
publique el evento:

1. Con `@EventListener` (listeners inmediatos)
    - Si el método publisher (ej. `changeOrderStatus(...)`) no es transaccional, entonces los listeners se anotan con
      `@EventListener`.
    - Esto significa que el listener se ejecutará tan pronto se publique el evento, sin importar si hay transacciones
      en curso.
    ````java
    @EventListener
    public void onEvent(OrderStatusChangedEvent event) {
        // Se ejecuta inmediatamente al publicarse el evento
    }
    ````

2. Con `@TransactionalEventListener` (listeners ligados a transacciones)
    - Si el método publisher es transaccional (`@Transactional`), entonces es recomendable usar
      `@TransactionalEventListener`.
    - Esto permite que el listener espere a que la transacción se confirme antes de ejecutarse.
    - Así se evita notificar acciones que podrían quedar inconsistentes si la transacción falla.
    ````java
    @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
    public void onEvent(OrderStatusChangedEvent event) {
        // Solo se ejecuta si la transacción se confirma
    }
    ````

| Escenario                   | Listener recomendado          | Momento de ejecución                                 |
|-----------------------------|-------------------------------|------------------------------------------------------|
| Método **NO transaccional** | `@EventListener`              | Al publicarse el evento                              |
| Método **Transaccional**    | `@TransactionalEventListener` | Según la fase de la transacción (ej. `AFTER_COMMIT`) |

👉 En conclusión:

- Usa `@EventListener` cuando no dependas de una transacción.
- Usa `@TransactionalEventListener` cuando quieras que tus listeners reaccionen solo si la transacción realmente se
  confirmó (lo más seguro en entornos productivos).

### 📩 Subscriber: EmailListener

A continuación se muestran los `subscribers` (o `listeners`) que reaccionan a los eventos publicados por el
`OrderService`.

El `EmailListener` es un `subscriber` que reacciona a los eventos de tipo `OrderStatusChangedEvent`.
Su función es enviar notificaciones por correo electrónico según el nuevo estado de la orden.
...
- `@Component`: lo registra como bean de Spring, permitiendo que se detecte automáticamente.
- `@EventListener`: indica que este método está suscrito al evento `OrderStatusChangedEvent`.
- `@Async("ordersExecutor")`: asegura que el procesamiento del evento sea asíncrono, utilizando el pool de hilos
  configurado (`ordersExecutor`).
- `switch (event.status())`: maneja la lógica en función del estado de la orden (`CONFIRMED`, `SHIPPED`, `CANCELLED`).

> 👉 Con esta configuración, el listener no bloquea el flujo principal, ya que los correos se procesan en segundo plano,
> de manera desacoplada al servicio que emitió el evento.

Los demás `listeners` (`InventoryListener`, `InvoiceListener`, `SmsListener`) siguen el mismo patrón, cada uno con su
responsabilidad concreta (gestión de stock, generación de factura, envío de SMS).

Esto refleja la desacoplación del patrón Observer: cada subscriber decide si actuar o no según el evento recibido, sin
que el `OrderService` tenga que conocerlos.
...
### 🌐 Publisher vía REST: `OrderRestController`

Este controlador expone un endpoint REST que permite a los clientes cambiar el estado de una orden. Internamente, delega
al `OrderService`, que además de realizar la lógica del negocio, publica un evento de dominio
(`OrderStatusChangedEvent`) para notificar a todos los suscriptores interesados.
...
👉 Gracias a este diseño, la API expone una interfaz sencilla para los clientes, mientras que el manejo de efectos
colaterales (emails, inventario, facturación, SMS, etc.) está completamente desacoplado y distribuido en listeners.

### Probando la aplicación: disparando eventos con curl

Para verificar que el patrón Observer funciona correctamente en nuestra aplicación Spring Boot, podemos enviar
solicitudes HTTP al `OrderRestController`. Cada petición cambia el estado de una orden y, automáticamente, se publican
los eventos que son procesados por los diferentes listeners de manera asíncrona.

- Ejemplo 1: Confirmar pedido

````bash
$ curl -v -X POST http://localhost:8080/api/v2/orders/ORD-001/status?status=CONFIRMED | jq
>
< HTTP/1.1 200
< Content-Length: 0
< Date: Fri, 22 Aug 2025 22:06:36 GMT
<
````

````bash
[nio-8080-exec-1] d.m.p.s.b.o.orders.service.OrderService  : Cambio de estado [pedido: ORD-001 -> CONFIRMED] (corrId=fa7a208440784bf3bed0e24749acf6d9)
[nio-8080-exec-1] d.m.p.s.b.o.orders.service.OrderService  : Publicando evento: CONFIRMED para pedido ORD-001
[       orders-3] d.m.p.s.b.o.o.listener.InvoiceListener   : Factura: Generando PDF para ORD-001 -> INV-ORD-001.pdf
[       orders-1] d.m.p.s.b.o.o.listener.EmailListener     : Email: Pedido confirm. ORD-001 (corrId=fa7a208440784bf3bed0e24749acf6d9)
[       orders-2] d.m.p.s.b.o.o.l.InventoryListener        : Inventario: Reduciendo stock para ORD-001
````

👉 Observamos cómo tres subscriptores diferentes reaccionan al mismo evento `OrderStatusChangedEvent`.

- Ejemplo 2: Enviar pedido (shipped)

````bash
$ curl -v -X POST http://localhost:8080/api/v2/orders/ORD-001/status?status=SHIPPED | jq
>
< HTTP/1.1 200
< Content-Length: 0
< Date: Fri, 22 Aug 2025 22:08:28 GMT
<
````

````bash
[io-8080-exec-10] d.m.p.s.b.o.orders.service.OrderService  : Cambio de estado [pedido: ORD-001 -> SHIPPED] (corrId=a5e82964c3824635b2fa98a85b327db9)
[io-8080-exec-10] d.m.p.s.b.o.orders.service.OrderService  : Publicando evento: SHIPPED para pedido ORD-001
[       orders-1] d.m.p.s.b.o.o.listener.EmailListener     : Email: Pedido ORD-001 enviado (corrId=a5e82964c3824635b2fa98a85b327db9)
[       orders-4] d.m.p.s.b.o.orders.listener.SmsListener  : SMS: Pedido ORD-001 despachado - Track: TRKORD-001
````

👉 Aquí los subscriptores `EmailListener` y `SmsListener` son los únicos interesados en reaccionar al estado `SHIPPED`.

- Ejemplo 3: Cancelar pedido

````bash
$ curl -v -X POST http://localhost:8080/api/v2/orders/ORD-001/status?status=CANCELLED | jq
>
< HTTP/1.1 200
< Content-Length: 0
< Date: Fri, 22 Aug 2025 22:07:35 GMT
<
````

````bash
[nio-8080-exec-6] d.m.p.s.b.o.orders.service.OrderService  : Cambio de estado [pedido: ORD-001 -> CANCELLED] (corrId=f97d432bba074715a0da28f16241e80c)
[nio-8080-exec-6] d.m.p.s.b.o.orders.service.OrderService  : Publicando evento: CANCELLED para pedido ORD-001
[       orders-3] d.m.p.s.b.o.o.listener.EmailListener     : Email: Pedido cancel. ORD-001 (corrId=f97d432bba074715a0da28f16241e80c)
[       orders-4] d.m.p.s.b.o.o.l.InventoryListener        : Inventario: Restaurando stock para ORD-001
````

👉 El evento `CANCELLED` activa solo a `EmailListener` e `InventoryListener`.

### 🎯 Conclusión final

Este flujo demuestra cómo el patrón Observer se implementa de manera natural en Spring Boot mediante:

- `Publisher (OrderService)` → Emite eventos de dominio con `ApplicationEventPublisher`.
- `Subscribers` (EmailListener`, `InventoryListener`, `InvoiceListener`, `SmsListener`) → Escuchan el evento y
  reaccionan de manera independiente.
- `Ejecución asíncrona` → Gracias a `@EnableAsync` y al `Executor` personalizado, cada listener procesa en paralelo,
  simulando un entorno real de microservicios.
- `Desacoplamiento` → El servicio principal (`OrderService`) no conoce a suscriptores; solo emite el evento. Esto hace
  al sistema extensible y mantenible.

> 👉 Así, logramos un ejemplo claro, sencillo, pero realista de cómo aplicar el patrón Observer en Spring Boot con
> eventos de dominio, y procesamiento asíncrono.
>
> 🔎 Nota: si incorporáramos métodos transaccionales (`@Transactional`), entonces correspondería usar
> `@TransactionalEventListener` para asegurar que los listeners se ejecuten únicamente después de un commit exitoso.
