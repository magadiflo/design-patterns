...
### 7. Solución de Singleton contra Reflexión

En la sección anterior vimos cómo `reflection` puede romper el patrón `Singleton`, permitiendo crear nuevas instancias
aunque el constructor sea privado.

Una de las soluciones más directas consiste en proteger el constructor privado, verificando si ya existe una instancia
creada. En caso afirmativo, se lanza una excepción para evitar que se construya otra.
...
- `if (instance != null) { throw new RuntimeException(...) }`:
    - Esta validación asegura que ningún proceso (incluyendo la reflexión) pueda llamar al constructor una segunda vez.
    - Si alguien intenta forzar la creación vía `Constructor.newInstance()`, inmediatamente se lanzará una excepción.
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // Intento de crear nueva instancia usando reflexión
...
````bash
19:52:22.016 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.reflection.fix.AppConfig -- Instancia creada
19:52:22.027 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.reflection.fix.Main -- 306206744
19:52:22.027 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.reflection.fix.Main -- 306206744
Exception in thread "main" java.lang.reflect.InvocationTargetException
	at java.base/jdk.internal.reflect.DirectConstructorHandleAccessor.newInstance(DirectConstructorHandleAccessor.java:74)
	at java.base/java.lang.reflect.Constructor.newInstanceWithCaller(Constructor.java:502)
	at java.base/java.lang.reflect.Constructor.newInstance(Constructor.java:486)
	at dev.magadiflo.patterns.plainjava.creational.singleton.implementations.reflection.fix.Main.main(Main.java:21)
Caused by: java.lang.RuntimeException: No se puede crear la instancia. Utilice getInstance() para crearla.
	at dev.magadiflo.patterns.plainjava.creational.singleton.implementations.reflection.fix.AppConfig.<init>(AppConfig.java:11)
	at java.base/jdk.internal.reflect.DirectConstructorHandleAccessor.newInstance(DirectConstructorHandleAccessor.java:62) 
````

#### Conclusiones

- Esta solución fortalece el patrón `Singleton` contra ataques con reflexión.
- Sin embargo, aunque es eficaz, no es completamente infalible: un atacante muy determinado podría usar otros mecanismos
  como `deserialización` o manipulación con librerías avanzadas de `bytecode` (ej. `Unsafe`).
- Por ello, más adelante veremos soluciones más robustas, como el uso de `enum singletons`, que ofrecen protección total
  contra reflexión y deserialización.

### 8.Violación de Singleton con Clone

El patrón Singleton garantiza que solo exista una única instancia de una clase durante la ejecución de la aplicación.
Sin embargo, si una clase Singleton implementa la interfaz `Cloneable` y no controla adecuadamente el método `clone()`,
es posible romper este contrato: al clonar el objeto se obtiene una nueva instancia independiente, violando el
principio del patrón.

#### Ejemplo: Singleton que permite ser clonado
...
public class MailSender implements Cloneable {
    ...
    protected Object clone() throws CloneNotSupportedException {
        return super.clone(); // genera una nueva instancia
    }
}
````
...
public class Main {
    public static void main(String[] args) throws CloneNotSupportedException {
        ...
        // Al clonar se obtiene una nueva instancia distinta
...
````bash
20:13:20.763 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.problem.MailSender -- Instancia creada
20:13:20.768 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.problem.Main -- 306206744
20:13:20.771 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.problem.Main -- 306206744
20:13:20.771 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.problem.Main -- 477289012
````

Explicación

1. `mailSender1` y `mailSender2` son iguales porque ambos provienen de `getInstance()`.
    - 👉 Tienen el mismo `identityHashCode`.

2. Al ejecutar `mailSender2.clone()`, se crea un nuevo objeto en memoria, lo que rompe la unicidad del Singleton.
    - 👉 El `identityHashCode` es distinto, demostrando que existen dos instancias.

#### ⚠️ Conclusión:

> El uso de `clone()` sin control permite que un Singleton pierda su naturaleza de instancia única.
> Esto es considerado una violación del patrón, similar a lo que ocurre con la reflexión.
> Más adelante veremos cómo proteger un Singleton de esta vulnerabilidad.

### 9. Solución a la violación de Singleton con Clone

Para proteger un Singleton de ser clonado indebidamente, debemos sobrescribir el método `clone()` y lanzar una excepción
cuando se intente duplicar la instancia.

De esta manera, se garantiza que la clase mantenga su unicidad incluso si implementa la interfaz `Cloneable`.

#### Ejemplo: Singleton protegido contra clonación
...
#### Prueba de seguridad contra clonación
...
````bash
20:22:32.498 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.fix.UserPreferences -- Instancia creada
20:22:32.503 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.fix.Main -- 1816089958
20:22:32.505 [main] INFO dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.fix.Main -- 1816089958
Exception in thread "main" java.lang.CloneNotSupportedException: No se puede crear la instancia. Utilice getInstance() para crearla.
	at dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.fix.UserPreferences.clone(UserPreferences.java:20)
	at dev.magadiflo.patterns.plainjava.creational.singleton.implementations.cloneable.fix.Main.main(Main.java:14)
Disconnected from the target VM, address: '127.0.0.1:52395', transport: 'socket'
````

Explicación

1. `Primera creación`: la instancia se inicializa normalmente con `getInstance()`.
2. `Clonación bloqueada`: al invocar `clone()`, se detecta que ya existe una instancia y se lanza la excepción
   `CloneNotSupportedException`.
3. Con esto, evitamos que se genere un nuevo objeto en memoria, asegurando la unicidad del Singleton.

#### ✅ Conclusión:

> Sobrescribir `clone()` para impedir la clonación es una defensa necesaria contra esta vulnerabilidad, al igual que la
> protección frente a la reflexión.
